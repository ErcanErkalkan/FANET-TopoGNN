...
def _throughput_split_metrics(
    ...
    if train.empty or test.empty:
        raise RuntimeError(f"Empty train/test partition for throughput split {split_name}")
    ...
    prediction_frame = test[["abs_time", "throughput"]].copy()
    prediction_frame["split"] = split_name
    prediction_frame["throughput_pred_rf"] = pred
    prediction_frame["throughput_pred_baseline"] = baseline
    return rows, prediction_frame


def _run_throughput(lte_path: Path, throughput_path: Path) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, float]]:
    ...
    split_specs = [
        ("final-30%-by-time", indices >= int(np.floor(0.7 * len(merged)))),
        ("first-half-to-second-half", indices >= len(merged) // 2),
    ]
    rows: list[dict[str, float | str]] = []
    predictions = []
    for split_name, test_mask in split_specs:
        split_rows, prediction_frame = _throughput_split_metrics(
            merged,
            feature_cols,
            split_name,
    ...
    diagnostics = {
        ...
        "throughput_lag1_autocorrelation": float(merged["throughput"].autocorr(lag=1)),
    }
    return pd.DataFrame(rows), pd.concat(predictions, ignore_index=True), diagnostics
...
def _plot_outputs(out_dir: Path, availability_records: dict[str, pd.DataFrame], throughput: pd.DataFrame) -> None:
    ...
    test = throughput[throughput["split"] == "final-30%-by-time"].dropna(subset=["throughput_pred_rf"])
...
def main() -> int:
    ...
    throughput_df, throughput_frame, throughput_diagnostics = _run_throughput(
    ...
    manifest = {
        ...
        "throughput_protocol": {
            "primary_split": "final-30%-by-time",
