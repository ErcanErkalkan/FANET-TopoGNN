...
The evidence includes simulated-radio experiments and a real-field-motion transfer test. The public flight bags do not contain measured RF or packet-reception labels, so their graph labels are counterfactual communication-radius sensitivities. Hardware-in-the-loop experiments, measured wireless links, packet-level medium-access validation, onboard profiling, and deployment-level validation remain outside the completed package.
